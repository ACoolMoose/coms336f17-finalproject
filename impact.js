...
function onDocumentMouseClick() {
    ...
    animate = $("#animateCheckbox").prop('checked');
    drawLines = $("#drawLinesCheckbox").prop('checked');
    experimental = $("#experimentalCheckbox").prop('checked');
    ...
    if (inWidth && inHeight) {
        ...
        for (i = 1; i <= numOfShatterLayers; ++i) {
            info("<b>Layer: " + (i + 1) + "</b>");
            if (experimental && i === numOfShatterLayers) {
                ...
                if (experimental) {
                    total = Math.pow(numOfShatterLayers, 2);
        ...
        for (var l = 1; l <= gL; ++l) {
            ...
            for (i = 0; i < LL; ++i) {
                ...
                if (experimental) {
                    ...
                    try {
                        t_mesh = new THREE.Mesh(new THREE.ShapeGeometry(shape), drawLines ? lineMaterial : objectMaterial);
                        scene.add(t_mesh);
                        fragments.push(t_mesh);
                    } catch (err) {
                        console.error(err);
...
function animateFragment(fragment, startPoint) {
    ...
    fragment['moveSpinPerc'] = 1 / (1 + Math.pow(distanceFromCenter, 2));
...
function handleKeyPress(event) {
    ...
    if (ch === 'H') {
        drawLines = !drawLines;
        $("#drawLinesCheckbox").bootstrapToggle('toggle');
    }
    ...
    if (ch === 'E') {
        ...
        $("#experimentalCheckbox").bootstrapToggle('toggle');
    }

    if (ch === ' ') {
        animate = !animate;
        $("#animateCheckbox").bootstrapToggle('toggle');
    }
...
function start() {
    ...
    if(animate) {
        $("#animateCheckbox").bootstrapToggle('on');
    } else {
        $("#animateCheckbox").bootstrapToggle('off');
    }

    if(drawLines) {
        $("#drawLinesCheckbox").bootstrapToggle('on');
    } else {
        $("#drawLinesCheckbox").bootstrapToggle('off');
    }

    if(experimental) {
        $("#experimentalCheckbox").bootstrapToggle('on');
    } else {
        $("#experimentalCheckbox").bootstrapToggle('off');
    }
    ...
    var render = function () {
        ...
        document.getElementById("frameNum").innerHTML = frameNumber.toString();
    };
